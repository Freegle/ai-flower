...
export class WorkflowEngine {
    ...
    constructor(config) {
        ...
        if (!result.valid) {
            throw new Error(`Invalid workflow definition: ${result.errors.map(e => e.message).join('; ')}`);
        }
    ...
    // ─── Instance management ──────────────────────────────────────────────────
    async createInstance(initialContext = {}, label) {
    ...
    async updateContext(instanceId, updates) {
     ...
     */
    async processInput(instanceId, input) {
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            const rawResponse = await this.config.llmAdapter.call(attempt === 0 ? system : `${system}\n\nPREVIOUS ATTEMPT ERROR: ${lastError}\n\nPlease fix your JSON response.`, user);
        ...
        const adapterWithUsage = this.config.llmAdapter;
     ...
     */
    async triggerTransition(instanceId, toState, metadata = {}) {
        const instance = await this.getInstance(instanceId);
        if (instance.status !== 'active') {
            throw new Error(`Instance '${instanceId}' is not active (status: ${instance.status})`);
        }
     ...
     */
    async forceTransition(instanceId, toState, reason) {
        const instance = await this.getInstance(instanceId);
    ...
    async processTimeouts() {
        ...
        for (const instance of instances) {
            if (!instance.timeoutAt)
                continue;
            if (new Date(instance.timeoutAt).getTime() > now)
                continue;
